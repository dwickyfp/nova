...
async def test_rule_proposal_requires_memory_in_active_role(
    ...
    monkeypatch.setattr(
        agent_router.agent_repository, "get_semantic_model",
        AsyncMock(return_value={"definition": sales_definition}),
    )
    monkeypatch.setattr(
        agent_router.agent_repository, "get_agent",
        AsyncMock(return_value={"semantic_model_ids": ["model-1"]}),
    )
...
async def test_rule_preview_executes_both_versions_under_active_role(
    ...
    monkeypatch.setattr(
        agent_router.agent_repository, "get_semantic_model",
        AsyncMock(return_value={"definition": sales_definition, "database_name": "sales_db"}),
    )
...
async def test_rule_approval_requires_preview_and_rejects_stale_model(
    ...
    update = AsyncMock()
    monkeypatch.setattr(agent_router.agent_repository, "update_semantic_model", update)
    ...
    with pytest.raises(HTTPException, match="semantic model changed"):
        await agent_router.approve_rule_proposal("model-1", "proposal-1", user)
    update.assert_not_awaited()
