...
Uses SHOW/DESC/INFORMATION_SCHEMA to discover databases, tables, views, columns.
All queries run as system admin (root) since metadata browsing needs full visibility.
"""
...
class ObjectRepository:
    ...
    async def get_database(self, name: str) -> dict | None:
        """Get database details."""
        result = await db.execute_system(f"SHOW CREATE DATABASE `{name}`")
        if not result["rows"]:
            return None
    ...
    async def list_tables(self, database: str) -> list[dict]:
        ...
        result = await db.execute_system(f"SHOW TABLES FROM `{database}`")
        tables = []
        for row in result["rows"]:
            name = row[0]
    ...
    async def get_table_detail(self, database: str, table: str) -> dict | None:
        ...
        try:
            desc_result = await db.execute_system(f"DESC `{database}`.`{table}`")
        except Exception:
        ...
        # DDL
        ddl_result = await db.execute_system(f"SHOW CREATE TABLE `{database}`.`{table}`")
        ddl = ddl_result["rows"][0][1] if ddl_result["rows"] else ""

        # Table status (engine, rows, size, etc.)
        status_result = await db.execute_system(f"SHOW TABLE STATUS FROM `{database}` LIKE '{table}'")
        status = {}
    ...
    async def list_views(self, database: str) -> list[dict]:
        ...
        for row in result["rows"]:
            ...
            try:
                view_result = await db.execute_system(f"SHOW CREATE VIEW `{database}`.`{name}`")
                if view_result["rows"]:
    ...
    async def get_view_detail(self, database: str, view: str) -> dict | None:
        ...
        try:
            ddl_result = await db.execute_system(f"SHOW CREATE VIEW `{database}`.`{view}`")
            ddl = ddl_result["rows"][0][1] if ddl_result["rows"] else ""
        except Exception:
            return None

        # Get column info
        desc_result = await db.execute_system(f"DESC `{database}`.`{view}`")
        columns = []
        for row in desc_result["rows"]:
            columns.append({
                "name": row[0],
    ...
    async def list_materialized_views(self, database: str) -> list[dict]:
        ...
        result = await db.execute_system(f"SHOW ALTER MATERIALIZED VIEW FROM `{database}`")
        mvs = []
        if result["rows"]:
            for row in result["rows"]:
    ...
    async def list_table_types(self, database: str) -> dict[str, list[dict]]:
        ...
        for row in result["rows"]:
            ...
            try:
                await db.execute_system(f"SHOW CREATE VIEW `{database}`.`{name}`")
                views.append({"name": name, "database": database, "type": "VIEW"})
            except Exception:
                tables.append({"name": name, "database": database, "type": "TABLE"})
        ...
        try:
            mv_result = await db.execute_system(f"SHOW ALTER MATERIALIZED VIEW FROM `{database}`")
            if mv_result["rows"]:
