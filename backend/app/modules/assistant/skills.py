...
_PRIMER_SPECS: tuple[_SectionSpec, ...] = (
    ...
    _SectionSpec(
        ...
        text=(
            ...
            "- `NOVA_SYSTEM.*`: ordinary StarRocks tables such as `ML_MODELS` "
            "and `AUDIT_LOG`. Verify exact names with authorized SHOW TABLES; "
            "design-document names are not proof that a table exists.\n"
            "- `LIST @stage/` lists files; `COPY INTO table FROM @stage.file` loads. Also use "
            "`INSERT INTO table SELECT ... FROM @stage.file.csv`."
        ),
    ),
    _SectionSpec(
        name="scope-boundary",
    ...
    _SectionSpec(
        ...
        text=(
            ...
            "a write, use query_mutate or a dedicated internal function "
            ...
            "'<temporary_password>' in account DDL and include ALTER USER ... "
            "REQUIRE PASSWORD CHANGE. provision_user collects protected input; `@stage` ensures they "
