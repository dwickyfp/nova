...
| **Reading `information_schema.task_runs` is served by the engine's internal task-history archive** (`_statistics_.task_run_history`, via `RepoExecutor`), and that read is **not** performed as the caller: the FE uses `ConnectContext.buildInner()`, so a non-admin submitter's grants do not apply to it. On a clean engine the archive table may be absent, and the read then fails with a `1064` (`RepoExecutorexecute sql failed: SELECT history_content_json FROM _statistics_.task_run_history …`) — for admins too, not just restricted users | The engine's task-run **surface is not a caller-scoped read**, so it must never be on the delegate-first path. Submit `SUBMIT TASK` on the owner's connection (RBAC is enforced there, at submit time, as `5203`), but read the watermark and poll for completion on the **system** connection: observing Nova's own orchestration state is Nova's bookkeeping, not the owner's. A failed watermark read is best-effort and a transient poll failure is retried, so an engine-side archive failure can never masquerade as a node's outcome. Verified on `4.1.1`; grant-based fixes were probed and are no-ops |
