...
- [ ] @stage SQL dialect — translator + credential injector exist, but the parse stage is regex-on-text and four NOVA-17 defects remain open. **Downgraded from `[x]` by NOVA-17** (audit on `237a64b`, re-confirmed at `f29c449`): `@@version` and a `@stage` ref inside a comment produce false-positive stage refs; `@stage1/folder/x.csv` (slash path) and the glob `@stage1.data/*.csv` are not detected. **Two NOVA-17 defects closed by NOVA-124**: `_normalize_default_schema_qualification` (`service.py:1460`, called at `:219` and `:965`) no longer corrupts `@stage1.data.default.csv` → `@stage1.data.csv` or `config.default.value` → `config.value` (positional anchor + literal/comment/stage masking, regression-locked in `backend/tests/unit/test_default_schema_normalization.py`); and `CREATE ML_MODEL ... TYPE = FORECAST` is accepted (`ml_engine` maps it onto the regression estimator family), matching `AGENTS.md:245` and `docs/19-machine-learning.md:184`. The `training_sql` path is also **no longer** raw — since NOVA-64 it runs through `_prepare_user_sql` → `guard_user_statement` plus `@stage` translation and credential injection, and only the redacted form is persisted (`ml_engine/service.py:148-153` calls `:717,725`), so that defect is closed. Remaining parse-stage defects are the ANTLR4 migration (NOVA-109).
...
- [x] ML model registry (classification, regression, forecast, anomaly_detection; 8 algorithms each) — `backend/app/modules/ml_engine/service.py`, tables `NOVA_SYSTEM.ML_MODELS` / `ML_MODEL_VERSIONS` / `ML_MODEL_ALIASES` (`docker/init-nova.sql:204-234`). **NOVA-124 added the documented `FORECAST` and `ANOMALY_DETECTION` types**: `CREATE ML_MODEL ... TYPE = FORECAST` now parses into `model_type="forecast"` (`backend/app/modules/query/dialect/ml_model.py`) and trains through the regression estimator family (`ALGORITHMS["forecast"]`); `ANOMALY_DETECTION` reuses the classifiers. The HTTP `POST /api/v1/ml/train` schema accepts the same four types (`backend/app/modules/ml_engine/schemas.py:16-23`). A dedicated time-series backend (Prophet, `SERIES`/`TIMESTAMP`/`CONFIG` clauses) is still **not** implemented — forecast reuses the regression estimators, which is the documented "fast" GBM path
