...
- [x] **9b** `CREATE TASK … AFTER / FINALIZE / WHEN / SCHEDULE` added to the existing ANTLR4 `submitTaskStatement` rule (NOVA-BEGIN/NOVA-END patch, `--fuzz=0`, CI drift check); it is a Nova surface, lowered to `SUBMIT TASK` — `fe/fe-grammar/src/main/antlr/com/starrocks/grammar/StarRocks.g4`, `backend/app/modules/task_orchestration/lowering.py` (PRs #57, #60, #61, #62)
