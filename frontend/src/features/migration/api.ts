...
export type PlanStepKind =
  "database" | "table" | "view" | "materialized_view" | "function" | "task";
...
export function testSourceConnection(input: {
  source: string;
  host: string;
  port: number;
  username: string;
  ...
  return api.post<{ connected: boolean }>(`${BASE}/sources/test`, input);
}

export type SourceDatabaseList = {
  source: string;
  databases: string[];
  count: number;
  unsupported?: string[];
...
export function fetchSourceDatabases(source: string) {
  return api.post<SourceDatabaseList>(`${BASE}/databases`, { source });
}
...
export type QueuedMigrationJob = Pick<
  MigrationJob,
  "job_id" | "status" | "source" | "databases"
>;

export function enqueueMigration(input: {
  source: string;
  databases: string[];
  create_database?: boolean;
  ...
  return api.post<QueuedMigrationJob>(`${BASE}/execute-batch`, input);
}

export function fetchMigrationJob(jobId: string) {
  return api.get<MigrationJob>(`${BASE}/jobs/${encodeURIComponent(jobId)}`);
}
