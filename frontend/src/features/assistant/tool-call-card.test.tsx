...
describe('ToolCallCard', () => {
  it('shows the SQL preview and a textual status label', async () => {
    const { getByText } = await render(
      <ToolCallCard toolCall={readOnly} toolCallId='tc-1' onDecide={() => {}} />
    )
    await expect
      .element(getByText('SELECT region, SUM(amount) FROM sales GROUP BY region'))
      .toBeInTheDocument()
  ...
  it('offers allow, deny and always-allow for a read-only statement', async () => {
    ...
    const { getByRole, getByText } = await render(
      <ToolCallCard toolCall={readOnly} toolCallId='tc-1' onDecide={onDecide} />
    )
  ...
  it('never offers always-allow for a destructive statement', async () => {
    ...
    const { getByRole, container } = await render(
      <ToolCallCard toolCall={destructive} toolCallId='tc-1' onDecide={onDecide} />
    )
  ...
  it('does not offer a decision while the tool call has no server id', async () => {
    const { container } = await render(<ToolCallCard toolCall={readOnly} onDecide={() => {}} />)
    expect(container.textContent).not.toContain('Deny')
  })
  ...
  it('surfaces a tool error with alert semantics', async () => {
    const { getByRole } = await render(
      <ToolCallCard
        toolCall={{ ...readOnly, status: 'failed', error: 'Permission denied by engine' }}
        toolCallId='tc-1'
        onDecide={() => {}}
      />
    )
