...
function SentAttachmentCard({ attachment }: { attachment: AttachedQuery }) {
  const meta = [attachment.database, attachment.schema].filter(Boolean).join(' · ')
  return (
    <span
    ...
    >
      ...
      <span className='min-w-0 flex-1'>
        <span className='block truncate font-medium'>{attachment.fileName || 'Selection'}</span>
...
function MessageBubble({
  ...
  const text = isUser ? (message.display_text ?? message.content) : message.content
    ...
    <div className={cn('flex', isUser ? 'justify-end' : 'justify-start')}>
      ...
      >
        {isUser && attachments.length > 0 ? (
          <div className='mb-2 flex flex-col gap-1.5' aria-label='Attached queries'>
        ...
        {isUser ? text : <Markdown runContext={runContext}>{message.content}</Markdown>}
