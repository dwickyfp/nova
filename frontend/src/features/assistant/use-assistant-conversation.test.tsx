...
function Harness({
  ...
  return (
    <span>
      ...
      <span data-testid="attachment-count">
...
describe("useAssistantConversation", () => {
  ...
  it("keeps attachments and the typed text on the stored user message", async () => {
    ...
    await expect
      .element(view.getByTestId("attachment-count"))
      .toHaveTextContent("1");
    await expect
      .element(view.getByTestId("display-text"))
      .toHaveTextContent("change it");
    // The model still receives the serialized preamble.
    await expect
      .element(view.getByTestId("content"))
      .toHaveTextContent("[Attached query");
  });
});
