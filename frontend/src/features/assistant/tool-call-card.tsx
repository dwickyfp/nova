...
export function ToolCallCard({ toolCall, toolCallId, onDecide, busy }: ToolCallCardProps) {
  const [alwaysAllow, setAlwaysAllow] = useState(false)
  const canOfferAlwaysAllow = toolCall.classification === 'read_only'
  const isDecidable = toolCall.status === 'pending' && Boolean(onDecide && toolCallId)

  const decide = (decision: 'approve' | 'deny') => {
    if (!toolCallId) return
    onDecide?.({ toolCallId, decision, alwaysAllow: alwaysAllow && canOfferAlwaysAllow })
  }
  ...
  return (
    ...
    >
      <div className='flex items-center justify-between gap-2'>
        <code className='truncate font-mono text-xs text-foreground'>{toolCall.tool_name}</code>
        <StatusBadge tone={STATUS_TONE[toolCall.status]} dot={toolCall.status === 'running'}>
      ...
      {toolCall.result_summary ? (
        <p className='mt-2 flex items-center gap-1.5 text-xs text-muted-foreground'>
          <Check aria-hidden='true' className='size-3.5 shrink-0 text-success-strong' />
          {toolCall.result_summary}
        </p>
      ) : null}
