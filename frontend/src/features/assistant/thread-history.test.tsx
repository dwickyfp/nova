...
describe('ThreadHistory', () => {
  it('does not fetch until the popover is opened', async () => {
    const { getByRole } = await renderHistory({ activeThreadId: null, onOpenThread: () => {} })
    const fetchMock = globalThis.fetch as unknown as ReturnType<typeof vi.fn>

    await expect.element(getByRole('button', { name: 'Chat history' })).toBeInTheDocument()
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/assistant/threads'))).toBe(
      false
    )
  })
})
