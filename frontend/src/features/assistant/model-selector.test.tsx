...
describe('listModelOptions', () => {
  it('keeps only active LLM models of providers that have an active key', async () => {
    ...
    expect(options).toEqual([
      { model: 'deepseek-v4', providerId: 'p1', providerName: 'Kenari', label: 'DeepSeek V4' },
    ])
  })
})
...
describe('ModelSelector', () => {
  it('shows the default label when nothing is selected', async () => {
    const { getByRole } = await renderSelector({ selected: null, onSelect: () => {} })
    await expect.element(getByRole('combobox', { name: 'Model' })).toHaveTextContent('Default model')
  })
  ...
  it('shows the selected model label once a choice is made', async () => {
    ...
    await expect.element(getByRole('combobox', { name: 'Model' })).toHaveTextContent('DeepSeek V4')
  })
