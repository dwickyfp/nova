...
describe("MessageList", () => {
  it("announces status transitions through a single polite live region", async () => {
    ...
    await expect
      ...
      .toHaveTextContent("Running query");
  });
  ...
  it("renders plain text without interpreting markup", async () => {
    ...
    await expect
      .element(getByText("<script>alert(1)</script>"))
      .toBeInTheDocument();
  ...
  it("renders an assistant answer as markdown", async () => {
    ...
    await expect
      .element(getByRole("heading", { name: "Result" }))
      .toBeInTheDocument();
        ...
        messages={[
          {
            ...userMessage,
            content:
              "[Attached query (file: Untitled-2.sql)]\n```sql\nSELECT * FROM t\n```\n\n---\n\nchange this to a cte",
  ...
  it("falls back to the wire text when a restored message has no attachment metadata", async () => {
    ...
    await expect
      .element(getByText("plain restored message"))
      .toBeInTheDocument();
  });
});
