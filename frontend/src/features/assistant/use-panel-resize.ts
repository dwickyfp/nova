 ...
 */
export function usePanelResize({ width, onCommit, onLiveWidth }: PanelResizeOptions) {
  ...
  const onPointerMove = useCallback(
    (event: PointerEvent) => {
      ...
      const next = clampAssistantWidth(state.startWidth + (state.startX - event.clientX))
