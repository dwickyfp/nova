...
export function AssistantComposer({
  ...
  const canSend = Boolean(value.trim()) && !streaming && !disabled && Boolean(onSendMessage)
        ...
        {attached.length > 0 ? (
          <div className='flex flex-wrap gap-1.5' aria-label='Attached queries'>
            {attached.map((attachment) => (
              ...
              >
                <FileCode aria-hidden='true' className='size-3 shrink-0 text-muted-foreground' />
                <span className='truncate font-medium'>{attachment.fileName || 'selection'}</span>
