...
const EMPTY_TURNS: AssistantTurnSnapshot = { threadId: null, grantActive: false }
    ...
    applyEvent: useCallback(
      (event: AssistantEvent) =>
        setStore((prev) => ({
          ...
          messages: {
            ...prev.messages,
            [binding]: transcriptReducer(prev.messages[binding] ?? [], { type: 'event', event }),
          },
        })),
      [binding]
    ),
    markCancelled: useCallback(
      () =>
        setStore((prev) => ({
          ...
          messages: {
            ...prev.messages,
            [binding]: transcriptReducer(prev.messages[binding] ?? [], { type: 'cancelled' }),
          },
        })),
      [binding]
    ),
