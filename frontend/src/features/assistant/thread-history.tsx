 ...
 */
export function ThreadHistory({ activeThreadId, onOpenThread, disabled }: ThreadHistoryProps) {
  ...
  return (
    <Popover
      ...
      onOpenChange={(next) => {
        setOpen(next)
        if (next) void queryClient.invalidateQueries({ queryKey: ['assistant-threads'] })
      }}
    >
                ...
                <li key={thread.thread_id}>
                  <button
                    ...
                    className={cn(
                      ...
                      thread.thread_id === activeThreadId && 'bg-accent text-accent-foreground'
                    )}
                  >
