...
export function ActivityTrace({ message }: ActivityTraceProps) {
  ...
  const summary = running ? 'Working' : `Thought for ${steps.length} step${steps.length === 1 ? '' : 's'}`
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {running ? (
          <Loader2 aria-hidden='true' className='size-3.5 shrink-0 animate-spin text-info-strong' />
        ) : (
          <Sparkles aria-hidden='true' className='size-3.5 shrink-0 text-info-strong' />
        )}
            ...
            <ol className='flex flex-col gap-0.5'>
              {plan.map((step) => (
                <li key={step.id} className='flex items-center gap-1.5'>
                  ...
                  <span className={cn(step.status === 'done' && 'text-foreground')}>
                    {step.text}
                  </span>
                </li>
              ))}
...
function StepMark({ status }: { status: 'pending' | 'running' | 'done' }) {
  if (status === 'running') {
    return (
      <Loader2 aria-hidden='true' className='size-3 shrink-0 animate-spin text-info-strong' />
    )
  }
  if (status === 'done') {
    return <span aria-hidden='true' className='size-3 shrink-0 text-success-strong'>✓</span>
  }
  return <span aria-hidden='true' className='size-3 shrink-0 rounded-full border' />
}
