...
describe('PanelResizeHandle', () => {
  it('exposes an accessible separator with the current width', async () => {
    const { getByRole } = await render(<PanelResizeHandle width={400} onResize={() => {}} />)
    ...
    expect(handle.element().getAttribute('aria-valuemin')).toBe(String(ASSISTANT_MIN_WIDTH))
    expect(handle.element().getAttribute('aria-valuemax')).toBe(String(ASSISTANT_MAX_WIDTH))
  })

  it('widens on ArrowLeft and narrows on ArrowRight', async () => {
    const onResize = vi.fn()
    const { getByRole } = await render(<PanelResizeHandle width={400} onResize={onResize} />)
    const handle = getByRole('separator', { name: 'Resize assistant panel' })

    await handle.element().dispatchEvent(
