 ...
 */
export function formatAttachmentsForPrompt(attachments: AttachedQuery[]): string {
  ...
  const blocks = attachments.map((attachment, index) => {
    const where = [
      ...
      .filter(Boolean)
      .join(', ')
    const label = attachments.length > 1 ? `Attached query ${index + 1}` : 'Attached query'
 ...
 */
export function buildRewriteHunks(before: string, after: string): RewriteHunk[] {
  ...
  while (index < ops.length) {
    ...
    if (deleted.length === 0) {
      hunks.push({ kind: 'insert', startLine: rangeStart - 1, endLine: rangeStart - 1, lines: inserted })
    } else {
