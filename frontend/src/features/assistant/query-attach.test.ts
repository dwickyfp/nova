...
describe("buildRewriteHunks", () => {
  ...
  it("collapses a changed line into one replace hunk", () => {
    ...
    expect(hunks).toEqual([
      { kind: "replace", startLine: 1, endLine: 1, lines: ["SELECT b"] },
    ]);
  });
  ...
  it("reports only the changed lines for a partial rewrite", () => {
    ...
    expect(hunks).toEqual([
      { kind: "insert", startLine: 4, endLine: 4, lines: ["WHERE a > 0"] },
    ]);
  });
  it("reports a deletion with no proposed lines", () => {
    const hunks = buildRewriteHunks(
      "SELECT 1\n-- comment\nFROM t",
      "SELECT 1\nFROM t",
    );
    expect(hunks).toEqual([
      { kind: "delete", startLine: 2, endLine: 2, lines: [] },
    ]);
  });
    ...
    expect(hunks).toEqual([
      {
        ...
        startLine: 1,
        endLine: 2,
        lines: ["SELECT a, b, c", "FROM big_table"],
      },
    ]);
  });
});
