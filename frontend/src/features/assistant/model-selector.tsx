...
export function ModelSelector({
  ...
  const selectedKey = selected ? `${selected.providerId}::${selected.model}` : undefined
  ...
  useEffect(() => {
    ...
    const stillValid = options.some((option) => optionKey(option) === selectedKey)
    if (stillValid) {
  ...
  const label = selected ? byKey.get(selectedKey ?? '')?.label ?? selected.model : 'Default model'
