      ...
      {open ? (
        <ScrollArea className="mt-4 min-h-0 flex-1 px-3">
          {/* Past conversations. Studio used to open every session empty, so a
              conversation you wanted to return to was unreachable. */}
          <div className="pb-3">
            <div className="flex items-center justify-between px-1 pb-1">
              ...
              <button
                type="button"
                onClick={onNewChat}
                className="rounded px-1 text-xs text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
              >
                New
              </button>
            </div>
                ...
                {threads.map((thread) => (
                  <li key={thread.thread_id}>
                    <button
                      ...
                      aria-current={
                        thread.thread_id === activeThreadId ? "true" : undefined
                      }
                      className={cn(
