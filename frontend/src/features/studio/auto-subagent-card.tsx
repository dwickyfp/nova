...
function Card({ root, children, agents, loading, error, retry, onSelectChild }: {
  ...
  return <section aria-label="Auto agent activity" className="flex max-h-[calc(100dvh-2rem)] min-h-0 flex-col overflow-hidden rounded-2xl border border-border bg-card text-card-foreground shadow-md">
    ...
    <div className="flex min-h-11 items-center gap-2 px-4 pb-3 text-sm"><span className="min-w-0 flex-1 font-medium">Main</span>{root ? <Status status={root.status} /> : <span className="text-xs text-muted-foreground">{error ? "Unavailable" : "Loading"}</span>}</div>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2 pt-3">
      ...
      {children.map((child) => {
        ...
        return <button key={child.run_id} type="button" onClick={() => onSelectChild(child.run_id)} aria-label={`Open ${name} conversation`} className="group flex min-h-14 w-full min-w-0 items-center gap-2 rounded-lg px-2 py-2 text-left transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{name}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{child.objective}</span></span><Status status={child.status} /><ChevronRight aria-hidden="true" className="size-3.5 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 motion-reduce:transition-none" />
        </button>;
      })}
    </div>
  </section>;
...
function ChildEvent({ event, rootRunId, childName, hasAnswer, toolNames }: { event: AutoRunEvent; rootRunId: string; childName: string; hasAnswer: boolean; toolNames: Map<string, string> }) {
  ...
  if (event.type === "agent_message") {
    ...
    const label = payload.origin === "user" ? `You to ${childName}` : sender === rootRunId ? `Main to ${childName}` : `${childName} to Main`;
  ...
  if (event.type === "child_activity") {
    ...
    if (["thinking", "plan"].includes(kind) && text) return <div className="min-w-0 border-l border-border pl-4 text-sm leading-relaxed text-muted-foreground"><span className="mb-1 block text-xs font-medium">{kind === "plan" ? "Plan" : "Working"}</span><p className="break-words whitespace-pre-wrap">{text}</p></div>;
...
export function AutoSubagentPanel({ selected, runs, agents, onSelectChild, onClose, onRefreshTree }: PanelProps) {
  ...
  const children = runs.filter((item) => item.depth === 1);
  ...
  const send = async () => {
    ...
    if (!selected || !active || !content || sending) return;
    setSending(true); setActionError(null);
    try { await agentsApi.sendAutoChildMessage(selected.rootRunId, selected.childRunId, { operation_id: crypto.randomUUID(), content }); setMessage(""); refresh(); }
  ...
  const cancel = async () => {
    ...
    setActionError(null);
    try { await agentsApi.cancelAutoChild(selected.rootRunId, selected.childRunId); refresh(); }
  ...
  return <aside
    ...
    className="flex min-h-0 min-w-0 w-full flex-1 flex-col overflow-hidden border-l border-border bg-background lg:w-[min(48rem,52%)] lg:flex-none"
  >
      <header className="shrink-0 border-b border-border">
        <div className="flex min-h-14 items-center px-3 sm:px-4">
          <div className="inline-flex min-w-0 items-center gap-2 rounded-xl bg-surface-1 py-1 pl-3 pr-1 text-sm font-medium">
            <Workflow aria-hidden="true" className="size-4 shrink-0 text-muted-foreground" strokeWidth={1.75} />
            <span>Subagents</span>
            <Button type="button" variant="ghost" size="icon" aria-label="Close subagent panel" onClick={onClose} className="ml-2 size-11 shrink-0 rounded-lg text-muted-foreground hover:text-foreground"><X aria-hidden="true" className="size-4" /></Button>
          </div>
        </div>
        <div className="flex min-h-16 min-w-0 items-center gap-3 px-3 sm:px-4">
          <Button type="button" variant="ghost" size="icon" aria-label={overview ? "Back to conversation" : "Back to subagents"} onClick={() => setOverview((current) => !current)} className="size-11 shrink-0 rounded-lg text-muted-foreground hover:text-foreground"><ArrowLeft aria-hidden="true" className="size-5" /></Button>
          <span aria-hidden="true" className="flex size-8 shrink-0 items-center justify-center rounded-lg border border-border bg-surface-1"><Workflow className="size-4 text-muted-foreground" strokeWidth={1.75} /></span>
          <h2 ref={titleRef} tabIndex={-1} className="min-w-0 flex-1 truncate text-base font-medium outline-none sm:text-lg">{overview ? "All subagents" : childName}</h2>
          {!overview && run ? <Status status={run.status} /> : null}
        </div>
      </header>
      {overview ? <div aria-label="Subagents" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-6">
        <p className="mb-3 text-xs text-muted-foreground">{activitySummary(children)}</p>
        <div className="space-y-1">
          {children.map((child) => {
            const name = child.agent_name || agents.find((agent) => agent.agent_id === child.agent_id)?.name || "Specialist";
            return <button key={child.run_id} type="button" aria-label={`Open ${name} conversation`} onClick={() => { if (child.run_id === selected.childRunId) setOverview(false); else onSelectChild(child.run_id); }} className="flex min-h-14 w-full min-w-0 items-center gap-3 rounded-lg px-3 py-2 text-left hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{name}</span><span className="block truncate text-xs text-muted-foreground">{child.objective}</span></span><Status status={child.status} /><ChevronRight aria-hidden="true" className="size-4 shrink-0 text-muted-foreground" /></button>;
          })}
          {children.length === 0 ? <p className="text-sm text-muted-foreground">No specialist assigned yet.</p> : null}
        </div>
      </div> : <>
      <div aria-label="Subagent timeline" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-6">
        ...
        {run ? <div className="mx-auto flex max-w-3xl flex-col gap-4"><article className="ml-5 rounded-xl border border-border bg-surface-1 px-4 py-3"><p className="mb-1 text-xs font-medium text-muted-foreground">Main assigned a task</p><p className="break-words whitespace-pre-wrap text-sm leading-relaxed">{run.objective}</p></article>
      ...
      <div className="shrink-0 border-t border-border bg-background px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 sm:px-6">
        ...
        {active ? <div className="mx-auto max-w-3xl"><div className="rounded-xl border border-border bg-card focus-within:ring-2 focus-within:ring-ring"><textarea aria-label="Message subagent" placeholder="Send context to this specialist" value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); void send(); } }} rows={2} maxLength={8000} className="max-h-40 w-full resize-none bg-transparent px-3 pt-3 text-sm outline-none placeholder:text-muted-foreground" /><div className="flex items-center justify-between px-2 pb-2"><Button type="button" variant="ghost" size="sm" onClick={() => void cancel()} className="min-h-11 text-muted-foreground hover:text-destructive"><Square aria-hidden="true" className="mr-2 size-3.5" />Cancel agent</Button><Button type="button" size="icon" aria-label="Send to subagent" disabled={!message.trim() || sending} onClick={() => void send()} className="size-10 rounded-full"><ArrowUp aria-hidden="true" className="size-4" /></Button></div></div></div>
          : run ? <p className="text-sm text-muted-foreground">This subagent has {statusLabel(run.status).toLowerCase()}. The conversation is read only.</p> : null}
      </div>
      </>}
    </aside>;
}
...
export function AutoSubagentCard({ runs, agents, loading, error, retry, onSelectChild }: CardProps) {
  const root = runs.find((run) => run.depth === 0), children = runs.filter((run) => run.depth === 1);
  ...
  return <><div className="absolute right-4 top-4 z-20 hidden w-[292px] xl:block">{body}</div><div className="shrink-0 px-4 pt-3 sm:px-6 xl:hidden"><div className="mx-auto max-w-3xl">{body}</div></div></>;
}
