        ...
        <CollapsibleContent className='CollapsibleContent'>
          <SidebarMenuSub>
            {item.items.map((subItem, index) => (
              <Fragment key={`${subItem.title}-${subItem.url}`}>
                {subItem.section && (
                  <SidebarMenuSubItem>
                    {index > 0 && (
                      <div aria-hidden='true' className='mx-2 my-2 h-px bg-sidebar-border' />
                    )}
                    <h3 className='px-2 py-1 text-xs font-medium text-sidebar-foreground/70'>
                      {subItem.section}
                    </h3>
                  </SidebarMenuSubItem>
                )}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side='right' align='start' sideOffset={4}>
          ...
          {item.items.map((sub, index) => (
            <Fragment key={`${sub.title}-${sub.url}`}>
              {sub.section && (
                <>
                  {index > 0 && <DropdownMenuSeparator />}
                  <DropdownMenuLabel>{sub.section}</DropdownMenuLabel>
                </>
              )}
