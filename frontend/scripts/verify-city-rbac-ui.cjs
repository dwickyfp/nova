...
const credentialFile = process.env.RBAC_DEMO_CREDENTIAL_FILE
  || path.resolve(__dirname, "../../workspace/rbac_city_demo/.env");
const passwords = Object.fromEntries(
  fs.readFileSync(credentialFile, "utf8").trim().split("\n").map((line) => {
...
async function checkUser(browser, username, city, amount) {
  ...
  try {
    ...
    await page.getByLabel("Password", { exact: true }).fill(passwords[username]);
    ...
    const ownAmounts = city === "Jakarta" ? ["100.00", "300.00"] : ["200.00", "400.00"];
    if (ownAmounts.some((value) => !workspaceText.includes(`${city}\t${value}`))) {
      throw new Error(`${username}: Workspace did not show the expected city amount`);
    }
    ...
    try {
      await page.getByText(city, { exact: true }).last().waitFor({ timeout: 60000 });
    } catch (error) {
    ...
    if (city === "Jakarta") {
      ...
      await page.getByText(
        "The authorized query returned no rows for this request.",
...
(async () => {
  ...
  try {
    ...
    console.log("PASS Jakarta asks Bandung in Studio: empty result and no browser errors");
  } finally {
    await browser.close();
  }
})().catch((error) => {
